# FILE: em-tools/emtools/translators/cibd25_importer.py
# ============================================================================
"""
CIBD25 Text to EMJSON v6 Translator

Main orchestrator for converting CIBD25 text format to EMJSON v6.
CIBD25 uses the same proprietary text-based format as CIBD22 with indentation,
for California Title 24 2025 compliance modeling.

Format characteristics:
- Text format with indentation (same as CIBD22)
- T24_2025.bin ruleset (vs T24N_2022.bin for CIBD22)
- CBECC 2025.1.0 software version
- Additional ProjVar and ResProj objects
- Compatible with CIBD22 parser infrastructure
"""
...
def _parse_du_types(parser, em: Dict[str, Any], id_registry: IDRegistry) -> Dict[str, str]:
    ...
    for du in du_types:
        ...
        # Generate stable ID
        du_id = id_registry.generate_id("DU", name, context="", source_format="CIBD25")
...
def _parse_window_types(parser, em: Dict[str, Any], id_registry: IDRegistry) -> Dict[str, str]:
    ...
    for wt in win_types:
        ...
        # Generate stable ID
        wt_id = id_registry.generate_id("WIN", name, context="", source_format="CIBD25")
...
def _parse_construction_types(parser, em: Dict[str, Any], id_registry: IDRegistry) -> Dict[str, str]:
    ...
    for ct in cons_types:
        ...
        # Generate stable ID
        ct_id = id_registry.generate_id("CONS", name, context="", source_format="CIBD25")
...
def _parse_materials(parser, em: Dict[str, Any], id_registry: IDRegistry) -> Dict[str, str]:
    ...
    for mat in all_mats:
        ...
        # Generate stable ID
        mat_id = id_registry.generate_id("MAT", name, context="", source_format="CIBD25")
...
def _parse_zones(parser, em: Dict[str, Any], id_registry: IDRegistry, 
    ...
    """Parse ResZn and ComZn objects with enhanced field coverage."""
    res_zones = parser.find_objects(obj_type="ResZn")
    com_zones = parser.find_objects(obj_type="ComZn")
    zones = res_zones + com_zones
    zone_list = []
    zone_name_to_id = {}
    ...
    for zn in zones:
        ...
        # Generate stable ID
        zone_id = id_registry.generate_id("Z", name, context="", source_format="CIBD25")
        zone_name_to_id[name] = zone_id
        
        # Parse DwellUnit reference
        du_ref = None
        du_count = 1
        ...
        # Parse floor area (ft² to m²) with DU type fallback
        floor_area_ft2 = _to_float(props.get("FloorArea")) if props.get("FloorArea") else None
        floor_area_m2 = None
        area_source = None
        
        if floor_area_ft2 and floor_area_ft2 > 0:
            floor_area_m2 = floor_area_ft2 * 0.092903
            area_source = "zone_property"
        elif du_ref and du_ref in du_id_to_data:
            # Fallback to DU type floor area
        ...
        # Parse heights (ft to m)
        ceiling_height_ft = _to_float(props.get("CeilingHeight"))
        ceiling_height_m = ceiling_height_ft * 0.3048 if ceiling_height_ft > 0 else None
        
        floor_height_ft = _to_float(props.get("FloorHeight"))
        floor_height_m = floor_height_ft * 0.3048 if floor_height_ft > 0 else None
        
        # Calculate volume if area and ceiling height available
        volume_m3 = None
        if floor_area_m2 and ceiling_height_m:
            volume_m3 = floor_area_m2 * ceiling_height_m
        
        # Parse zone type
        zone_type = props.get("Type", "Conditioned")
...
def _parse_surfaces(parser, em: Dict[str, Any], id_registry: IDRegistry,
    ...
    from eco_tools.parsers.cibd22_name_resolver import CIBD22NameResolver
    ...
    for obj_type, bucket in type_to_bucket.items():
        ...
        for surf in surf_objs:
            ...
            # Generate stable ID
            surf_id = id_registry.generate_id("S", name, context=zone_id or "", source_format="CIBD25")
...
def _parse_hvac_systems(parser, em: Dict[str, Any], id_registry: IDRegistry) -> None:
    ...
    for hvac in hvac_objs:
        ...
        # Generate stable ID
        hvac_id = id_registry.generate_id("HVAC", name, context="", source_format="CIBD25")
...
def _parse_dhw_systems(parser, em: Dict[str, Any], id_registry: IDRegistry) -> None:
    ...
    for dhw in dhw_objs:
        ...
        # Generate stable ID
        dhw_id = id_registry.generate_id("DHW", name, context="", source_format="CIBD25")
...
def translate_cibd25_to_v6(file_path: str) -> Dict[str, Any]:
    """
    Translate CIBD25 text format to EMJSON v6.
...
def main(argv=None):
    ...
    if not argv:
        print("Usage: python -m emtools.translators.cibd25_importer <input.cibd25> [output.emjson]")
        return 2
    
    out = argv[1] if len(argv) > 1 else argv[0].rsplit('.', 1)[0] + ".emjson"
    em = translate_cibd25_to_v6(argv[0])
